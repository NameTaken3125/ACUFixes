...
{
public:
    SharedPtrNew<SkeletonComponent>* SkeletonComponent_; //0x0000
    uint32 BoneID; //0x0008
    uint16 BoneIndex_idxInSkeletonCpntBoneInstances; //0x000C
    uint16 SkeletonVersion; //0x000E
}; //Size: 0x0010
assert_sizeof(BoneHandle, 0x10);
class WindForce
{
public:
    char pad_0000[24]; //0x0000
}; //Size: 0x0018
assert_sizeof(WindForce, 0x18);

class SkeletonPoseBlender;
class ReflexSystemData;
class SkeletonComponent : public Component
{
public:
    Vector4f WindForceValue; //0x0020
    SharedPtrNew<Skeleton>* MainSkeleton; //0x0030
    SmallArray<SharedPtrNew<Skeleton>*> AddonSkeletons; //0x0038
    char pad_0044[4]; //0x0044
    SharedPtrNew<ExpressionPack>* ExpressionPack_; //0x0048
    SmallArray<SkeletonPoseBlender> PoseBlenders; //0x0050
    SmallArray<BoneHandle> BoneInstances; //0x005C
    SmallArray<BoneHandle> WrinkleBoneInstances; //0x0068
    char pad_0074[4]; //0x0074
    SharedPtrNew<LookAtDataGeneral>* LookAtDataGeneralDefault; //0x0078
    LookAtData* LookAtData_; //0x0080
    SmallArray<ReflexSystemData*> ReflexInstanceData; //0x0088
    char pad_0094[4]; //0x0094
    void* IKGround_; //0x0098
    MotionSmoother* MotionSmoother_; //0x00A0
    SharedPtrNew<SkeletonComponent>* selfSharedPtr; //0x00A8
    SmallArray<BallJointChain*> arrBallJointChains; //0x00B0
    uint32 CurrentActiveExpressionID; //0x00BC
    uint32 CurrentExpressionIntensity; //0x00C0
    uint32 SkeletonDescKey; //0x00C4
    uint32 SkeletonHierarchyKey; //0x00C8
    uint32 BoneLODKey; //0x00CC
    WindForce WindForce_; //0x00D0

    uint16 ValidLODLevel : 3;
    uint16 StoredLODLevel : 3;
    uint16 ResolveLODLevel : 3;
    uint16 EnableDisplacement : 1;
    uint16 DynamicBV : 1;
    uint16 bits_E8_0xB_0x10 : 0x10 - 0xB;
    ...
    SmallArray<BoneHandle> arr494; //0x0494
    ...
    uint8 byte_CD0_0xC0ResolveAllBoneHierarchy; //0x0CD0
    uint8 byte_CD1; //0x0CD1
    char pad_0CD2[14]; //0x0CD2
}; //Size: 0x0CE0
assert_offsetof(SkeletonComponent, criticalSection_240, 0x240);

...
class JSON
{
    ...
    public:
		...
		// Like `dump()` but without unnecessary whitespace.
...
namespace {
	...
	inline JSON parse_object( const string &str, size_t &offset ) {
        ...
        while( true ) {
            ...
            // FIX: Allow trailing comma.
            // WARNING: Don't use repeated commas, though. The parsing fails at that point,
            //          just like it does without the fix.
            if( str[offset] == ',' ) {
                ++offset;
                consume_ws(str, offset);
                if (str[offset] == '}')

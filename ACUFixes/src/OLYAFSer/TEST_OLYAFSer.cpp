...
{
    ConfigTop cfg;
    BindableKeyCode_Keyboard c = cfg.hacks->enterWindowsByPressingAButton->enterWindowsButton;
    JSON jsonOut;
    cfg.SectionToJSON(jsonOut);
    ImGui::Text(jsonOut.dump().c_str());
    cfg.hacks->enterWindowsByPressingAButton->enterWindowsButton = BindableKeyCode_Keyboard::K_COMMA;
    ImGui::Separator();
    cfg.SectionToJSON(jsonOut);
    ImGui::Text(jsonOut.dump().c_str());
}
